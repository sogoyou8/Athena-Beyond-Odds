...
L'apparence s'adaptera initialement à la préférence système (`prefers-color-scheme`) avec option de bascule manuelle en session. Le CSS natif utilisera exclusivement des variables tokens neutres techniques provisoires (ex: `--color-surface-base`, `--font-family-base`). La palette hexadécimale de marque finale, les typographies commerciales dédiées, le logo officiel et les icônes propriétaires restent strictement non décidés et interdits.
